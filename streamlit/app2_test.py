...
# ---------------------------------------
    ...
    tabs_D = st.tabs(["타사이전 지표", "TOP100 / VIP 관리몰","카테고리별 매출", "카페24 EC 전체"])
        ...
        with tabs_D[1]:
            ...
            with tabs_D[2]:
                ...
                <div style="
                    ...
                    line-height:1.2;
                ">
                    <span style="font-size:48px; display:block;">10.6%</span>
                ...
                ">
                    <p style="margin:0;">
                    <strong>25년 2월</strong>은 전년 동월 대비 
                    <strong style="color:#FFD700;">10.6%</strong> 상승하며, 
                    전월 대비 YoY도 소폭 상승하였습니다. 전반적으로 모든 카테고리 매출이 상승하여 
                    특히 <strong>식품</strong>, <strong>생활/건강</strong> 카테고리에서 매출을 견인한 반면, 
                    <strong>패션의류</strong>는 -1.3% 하락하였습니다.
                    </p>
                </div>
                </div>
                """, unsafe_allow_html=True)

                # 예시용 임의 데이터 (실제 스프레드시트 데이터로 교체 가능)
                df_ec = pd.DataFrame({
                    "Month": ["2023-01", "2023-02", "2023-03", "2023-04", "2023-05"],
                    "Sales": [100, 110, 105, 120, 130],  # 단위: 억원
                })

                df_category_yoy = pd.DataFrame({
                    "Category": ["식품", "생활/건강", "화장품/미용", "출산/육아", "패션잡화", "패션의류", "종합몰"],
                    "YoY(%)": [18.8, 22.6, 15.1, 26.9, 24.3, 1.3, 23.0],
                })

                # ─────────────────────────────────────────────────────
                # 1) 레이아웃: 왼쪽(선 그래프) : 오른쪽(YoY) = 3 : 1
                # ─────────────────────────────────────────────────────
                col_left, col_right = st.columns([3, 1])

                with col_left:
                    # (A) 월별 거래액 추이 (선 그래프)
                    st.markdown("### 월별 EC 전체 거래액 추이 (단위: 억원)")
                    fig_line = px.line(
                        df_ec, 
                        x="Month", 
                        y="Sales", 
                        markers=True,
                        title="",  # 제목은 별도로 표기하므로 빈 값
                    )
                    fig_line.update_layout(height=400)
                    st.plotly_chart(fig_line, use_container_width=True)

                with col_right:
                    # (B) 카테고리별 YoY 리스트 (2개씩 가로로 표시)
                    st.markdown("### 카테고리별 YoY")

                    # YoY 값에 따라 ▲/▼, 색상 등 결정
                    # 필요하면 CSS로 꾸미거나 badge 형태로 만들 수 있음
                    chunk_size = 2  # 한 행에 2개씩
                    for i in range(0, len(df_category_yoy), chunk_size):
                        row_data = df_category_yoy.iloc[i : i + chunk_size]
                        yoy_cols = st.columns(chunk_size)
                        
                        for col_idx, row in enumerate(row_data.itertuples()):
                            cat_name = row.Category
                            yoy_val = row._2  # row["YoY(%)"]가 안 되므로, namedtuple 인덱스 또는 getattr 사용
                            arrow_symbol = "▲" if yoy_val >= 0 else "▼"
                            arrow_color = "red" if yoy_val >= 0 else "blue"

                            yoy_cols[col_idx].markdown(f"""
                            <div style="border:1px solid #ddd; border-radius:5px; padding:10px; margin-bottom:10px;">
                                <div style="font-weight:bold; font-size:16px;">{cat_name}</div>
                                <div style="color:{arrow_color}; font-size:18px;">
                                    {arrow_symbol} {abs(yoy_val)}%
                                </div>
                            </div>
                            """, unsafe_allow_html=True)

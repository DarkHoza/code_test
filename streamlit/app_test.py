...
if current_page == "category":
    ...
    .badge {
        ...
        padding: 6px 13px;  /* 배지 크기 약간 증가 */
        border-radius: 15px;
        color: #fff;
        font-size: 14px;  /* 배지 폰트 크기 증가 */
        margin: 2px;      /* 배지 간격 조정 */
    }
    .badge.blue {
    ...
    # -----------------------------------------------------
    topic = st.sidebar.selectbox("주제 선택", ["종합 대시보드","Data-eye 대시보드","Topic A", "Topic B", "Topic C" ])
    ...
    # ---------------------------------------
        ...
        try:
            ...
            # 탭 D[1]: TOP100 / VIP 관리몰 탭의 카드 영역 수정 (조 단위 변환 적용)
            with tabs_D[1]:
                ...
                if 'df_sales' in st.session_state and not st.session_state['df_sales'].empty:
                    ...
                    col_text, col_btn = st.columns([3, 1])  # 왼쪽 3, 오른쪽 2 비율로 열 생성
                    ...
                    with col_btn:
                        ...
                        <div class="button-container">
                            <a href="?page=category" target="_self" class="button-style">📊 카테고리별 매출</a>
                            <a href="http://example.com/data_source" target="_blank" class="button-style">📂 데이터원본</a>
                        </div>
                        """, unsafe_allow_html=True)
                ...
                with col1:
                    ...
                    """, unsafe_allow_html=True)
                    # TOP100 / VIP 관리몰 탭에 원본 데이터 expander 추가
                with st.expander("월별 매출 원본 데이터"):
                        # 1) 원본 데이터 중 필요한 컬럼만 복사
                ...
                # 3) 원하는 연도만 필터 (예: 2024, 2025)
                df_filtered = df_chart[df_chart['Year'].isin([2024, 2025])]

                # 4) melt로 VIP, TOP100 컬럼을 하나로 통합
                df_melted = df_filtered.melt(
                    id_vars=['Year', 'Month'],
                    value_vars=['VIP', 'TOP100'],
                    var_name='Metric',     # 'VIP' or 'TOP100'
                    value_name='Value'
                )

                # 5) Year와 Metric을 합쳐서 하나의 범례 항목으로 사용
                df_melted['Year_Metric'] = df_melted['Year'].astype(str) + '_' + df_melted['Metric']
                # 예: "2024_VIP", "2024_TOP100", "2025_VIP", "2025_TOP100"

                # 6) 라인 차트 생성 (x=Month, y=Value, color=Year_Metric)
                fig = px.line(
                    df_melted,
                    x='Month',
                    y='Value',
                    ...
                    color_discrete_map={
                        "2024_VIP": "rgba(255,165,0,0.3)",      # 오렌지(orange), 투명도 0.8
                        "2024_TOP100": "rgba(135,206,250,0.5)",  # 하늘색(skyblue), 투명도 0.8
                        "2025_VIP": "rgba(255,165,0,1)",
                        "2025_TOP100": "blue"
                        }
                    )
                # 기준 월 강조 코드: ref_month가 이미 정의되어 있어야 합니다.
                ref_month_int = int(ref_month)  # 예: 기준 월 "2"인 경우
                fig.add_vrect(
            ...
            # 탭 D[2]: 카페24 EC 전체 탭의 카드 영역과 차트(실제 데이터 연결)
                with tabs_D[2]:
                    ...
                    if 'df_sales' in st.session_state and not st.session_state['df_sales'].empty:
                        ...
                        with col1:
                            ...
                            last_year_change = total_revenue - float(last_year_same_month['전체']) if last_year_same_month is not None else 0
                            last_year_percent = (last_year_change / float(last_year_same_month['전체']) * 100) if last_year_same_month is not None and float(last_year_same_month['전체']) != 0 else 0
                        ...
                        # 카페24 EC 전체 차트 (3년치: 2년 전, 전년, 올해)
                        if df_sales is not None:
                            df_chart_ec = df_sales.copy()
                            # 1) '전체' 컬럼을 원에서 '억' 단위로 변환
                            df_chart_ec['전체'] = df_chart_ec['전체'] / 100_000_000

                            # 2) x축용 datetime 컬럼 생성 및 정리
                            ...
                            # 5) x축은 월(1~12)만 표시, 연도별 선을 color로 구분
                            fig_ec = px.line(
                            df_filtered_ec,
                            x='Month',
                            y='전체',
                            ...
                            # 2) 기준 월 강조
                            ref_month_int = int(ref_month)  # 예: "2" → 2
                            fig_ec.add_vrect(
                                x0=ref_month_int - 0.5,
                                x1=ref_month_int + 0.5,
                                fillcolor="LightSalmon",
                        ...
                        fig_ec.update_yaxes(
                            tickformat=".0f",  # 정수 형태로 표시 (소수점 없이)
                            ticksuffix="억"    # 숫자 뒤에 "억"을 붙임
    )
